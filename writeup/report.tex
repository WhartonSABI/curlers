...
Figure~\ref{fig:pp_eval_heatmaps} shows that teams are usually close to optimal in early ends, but diverge sharply in late-game, high-leverage states. The largest average losses occur in end 8 when the game is close (tied or trailing narrowly), consistent with Figure~\ref{fig:policy}, where $\Delta$WP is largest in those same states.
...
Our decision-quality analysis shows substantial room for improvement in late-game situations. For example, in end 8 when trailing by 1 or 2 with hammer and Power Play available, deviations from the optimal policy correspond to large average win-probability losses (on the order of 10 percentage points in the highest-leverage cells in Figure~\ref{fig:pp_eval_heatmaps}). These are exactly the states where the policy heatmaps show the largest value of using Power Play immediately.
